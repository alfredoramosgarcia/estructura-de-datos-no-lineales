...
public:
typedef size_t nodo; // Índice del vector entre 0 y maxNodos-1static const nodo NODO_NULO;
static const nodo NODO_NULO;

explicit Abin(size_t maxNodos); // Ctor., requiere ctor. T()void 
...
// Actualizar la posición del hijo (izquierdo o derecho)// en el padre del nodo movido.
if (nodos[nodos[hdrcho].padre].hizq == numNodos-1)nodos[nodos[hdrcho].padre].hizq = hdrcho;
else
...
int Abin<T>::profundidadNodo(nodo n){
    ...
    if (n == Abin<T>::NODO_NULO)
        ...
        return 1 + profundidadNodo(padre)
}

template <typename T>
int Abin<T>::alturaNodo(nodo n){
    nodo hijoD = nodos[n].hder;
    nodo hijoI = nodos[n].hizq;

    if(n == Abin<T>::NODO_NULO)
        return -1;
    else
        return 1 + std::max(alturaNodo(hijoI), alturaNodo(hijoD));
}
